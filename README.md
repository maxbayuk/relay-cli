...
With an API key (optional — enables rate limit increases and authenticated endpoints):
...
| `agents/tool-catalog.json` | All 21 commands with parameter schemas and types |
| `agents/error-catalog.json` | 8 error categories with retry strategies |
...
relay schema --list          # All 21 public endpoints with methods
```
...
3. **Dry-run support** — `--dry-run` on any POST prints the curl equivalent for review before committing.
...
21 commands across 7 groups, auto-generated from the OpenAPI spec. New endpoints appear automatically without code changes.
...
| Guard | What it does |
|-------|-------------|
| Read + quote only | Covers discovery, quoting, and tracking. Signing is handled externally by your app or wallet |
...
# 2. Preview the request (dry run)
relay quote --params '{"user":"0x...","originChainId":8453,"destinationChainId":1,"originCurrency":"0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913","destinationCurrency":"0x0000000000000000000000000000000000000000","amount":"1000000"}' --dry-run

# 3. Use the quote response to build your transaction externally
echo "$QUOTE" | jq '.steps'
```
...
2. Filters to the 21 public read/quote/tracking endpoints
